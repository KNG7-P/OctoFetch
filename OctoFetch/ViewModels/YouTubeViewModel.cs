...
namespace OctoFetch.ViewModels
{
    public partial class YouTubeViewModel : ObservableObject
    {
        ...
        /// <summary>Called when user triggers a YouTube download (videoUrl, videoTitle, format, quality).</summary>
        ...
        private async Task SearchAsync()
        {
            ...
            try
            {
                ...
                {
                    await BrowseChannelCoreAsync(channelId, ct).ConfigureAwait(true);
                    return;
                }
        ...
        private async Task BrowseChannelAsync(string? channelId)
        {
            ...
            _searchCts?.Cancel();
            _searchCts = new CancellationTokenSource();
            await BrowseChannelCoreAsync(channelId, _searchCts.Token).ConfigureAwait(true);
        }

        private async Task BrowseChannelCoreAsync(string channelId, CancellationToken ct)
        {
